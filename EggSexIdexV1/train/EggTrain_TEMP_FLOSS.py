...
from train.module_dataset.CNN1DModule_SNV_CBAM_WITH_T import *
    ...
    for i in range(20):
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc and val_acc > 0.9:
                ...
                model_path = os.path.join(fold1, f'1DCNN_acc_{best_val_acc:.6f}_vloss_{best_val_loss:.6f}.pth')
                ...
                with open(model_path.replace(".pth", "_temperature.txt"), "w") as f_temp:
                    f_temp.write(str(temp_value))
                print(f"保存模型及温度: {model_path}, 温度={temp_value:.4f}")
            ...
            # ====== 保存F1分数最高的模型 + 温度 ======
            if female_recall > 0.96 and female_f1 > 0.92 and female_f1 > max_f1:
                max_f1 = female_f1
                model_path = os.path.join(fold1, f'1DCNN_f1_{max_f1:.6f}_vacc_{val_acc:.6f}.pth')
                torch.save(model.state_dict(), model_path)
            ...
            if model_path and os.path.exists(model_path):
                try:
                    ...
                    test(model, test_loader, criterion, device,
                         thresholds=[0.15, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.6, 0.7],
...
if __name__ == '__main__':
    ...
    fold = r'D:\workspace\gdv-egg-model\Code_EggGenderDet\1DCNN_200epoch_SNV\huayu_715_hlh_T_FLoss_SG'  # 母多公少，half，16*16
    EggNetMain()
