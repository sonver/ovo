...
class HyperspectralCNN(nn.Module):
    def __init__(self, num_bands=150):
    ...
    def forward(self, x):
        ...
        # 重塑为 (batch_size * num_pixels, 1, num_bands)
        x = x.view(batch_size * num_pixels, 1, num_bands)

        # 1D CNN
        x = F.relu(self.bn1(self.conv1(x)))  # (batch_size * num_pixels, 64, 75)
        x = self.cbam1(x)  # 应用 CBAM 注意力
        x = self.pool(x)  # (batch_size * num_pixels, 64, 37)
        x = self.res1(x)
        x = self.res2(x)
        x = self.cbam2(x)  # 应用 CBAM 注意力
        x = self.pool(x)  # (batch_size * num_pixels, 2048, 9)
        x = self.dropout(x)
        x = self.avg_pool(x)  # (batch_size * num_pixels, 2048, 1)
        x = x.view(batch_size * num_pixels, -1)  # (batch_size * num_pixels, 2048)

        # 重塑为 (batch_size, num_pixels, 32)
        x = x.view(batch_size, num_pixels, -1)
        # 平均所有像素特征
        x = x.mean(dim=1)  # (batch_size, 32)
        # 分类
...
class CustomDataset_Meng(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        snv_data = np.apply_along_axis(snv_normalize, 1, data)  # [num_pixels, num_bands]

        # 2) SG (建议deriv=1 或 0，根据实验)
        sg_data = savgol_filter(snv_data, window_length=9, polyorder=2, deriv=1, axis=1)

        # 3) pooling: mean 或 median
        mean_spectrum = np.median(sg_data, axis=0) # sg_data.mean(axis=0)  # 或 np.median(sg_data, axis=0)

        # 4) 中心化
        result = mean_spectrum.reshape(1, -1)
        result = result - result.mean()
        data_tensor = torch.tensor(result, dtype=torch.float32).unsqueeze(0)
        ...
        # result = []
        # sigma = 1.0
        # for col in range(sg_data.shape[1]):
        #     column_data = sg_data[:, col]
        ...
        # result = np.array(result).reshape(1, -1)  # [1, num_bands]
