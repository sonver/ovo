...
class ModelWithTemperature(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model
        self.temperature = nn.Parameter(torch.ones(1) * 1.0)  # 初始温度=1
    ...
    def set_temperature(self, valid_loader, device):
        ...
        with torch.no_grad():
            for inputs, labels, _ in valid_loader:
                ...
                logits = self.model(inputs)   # 注意这里用原始 model，不带温度
...
class CustomDataset_Meng(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # sg_data = savgol_filter(snv_data, window_length=11, polyorder=2, axis=1)
        # result = []
        # sigma = 1.0
