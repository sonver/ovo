...
from train.module_dataset.CNN1DModule_SNV_CBAM_WITH_T_HLH import *
        ...
        for model_type, file_name in [('val_acc', 'test_acc.txt'), ('f1_score', 'test_recall_f1.txt')]:
            ...
            if model_path and os.path.exists(model_path):
                try:
                    ...
                    # 在校准温度后，保存温度参数
                    temperature_value = model_with_temp.temperature.item()
                    temperature_filename = os.path.join(fold1,
                                                        f'1DCNN_acc_{best_val_acc:.6f}_vloss_{best_val_loss:.6f}_temperature.txt' if model_type == 'val_acc' else
                                                        f'1DCNN_f1_{max_f1:.6f}_vacc_{best_val_acc:.6f}_temperature.txt' if max_f1 else '')
                    with open(temperature_filename, 'w') as f_temp:
                        f_temp.write(str(temperature_value))
                    print(f"保存温度参数 {temperature_value:.4f} 到 {temperature_filename}")

                    print(f"测试模型 (with temperature scaling): {model_path}")
                    test(model_with_temp, test_loader, criterion, device,
...
def to_torchScript():
    ...
    root_path = r'D:\workspace\gdv-egg-model\Code_EggGenderDet\1DCNN_200epoch_SNV\mixed_811_hlh_T\2'
    try:
        model = HyperspectralCNN(num_bands=150, use_attention_pooling=True).to(device)
        model.load_state_dict(torch.load(os.path.join(root_path, '1DCNN_acc_0.952120_vloss_0.298265.pth')))
        torch.jit.script(model).save(os.path.join(root_path, 'HyLine_Mixed_Acc937.pt'))
    except Exception as e:
        print(f"模型转换失败: {e}")
...
if __name__ == '__main__':
    ...
    female_samples, male_samples = 2443, 2529
    female_weight_times = 1.3
    n_bands = 150

    # 1.2倍的female权重放大 + 注意力池化 + 温度缩放
    data_folder = r"D:\workspace\gdv-egg-model\Code_EggGenderDet\data_hlh_train\Dataset\huayu"
    fold = r'D:\workspace\gdv-egg-model\Code_EggGenderDet\1DCNN_200epoch_SNV\huayu_hlh_811_T'
    EggNetMain()
