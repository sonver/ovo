...
def getsex202412156(root_path, source_folder, destination_folder3,
                    destination_folder4):
    # 创建目标文件夹（如果不存在）
    os.makedirs(destination_folder3, exist_ok=True)
    os.makedirs(destination_folder4, exist_ok=True)
    ...
    for txt_file in txt_files:
        ...
        match = re.match(r'egg([0-9A-Za-z]+)-(\d+)\.txt', txt_file)
        if not match:
        ...
        if X_str in df1.columns:
            ...
            if 0 <= Y - 1 < len(df1):
                ...
                if result == 3:  # 浅色母蛋
                    ...
                    print(f"文件 '{txt_file}' 的值 '{result}' 不是 1 2 3 4中的任何一个，所以忽略")
            else:
...
if __name__ == '__main__':
    # Excel 表路径
    root_path = r'D:\workspace\gdv-egg-model\Code_EggGenderDet\data_hlh_train\xiaoming_hll.xlsx'
    # 源文件夹和目标文件夹
    source_folder = './data_hlh_train/egg_seg_data_xiaoming'
    # destination_folder1 = './data/Dataset/only34/male'  # 公蛋存放目录
    # destination_folder2 = './data/Dataset/only34/female'  # 母蛋存放目录
    destination_folder3 = './data_hlh_train/Dataset/only34/female'  # 浅色母蛋存放目录
    destination_folder4 = './data_hlh_train/Dataset/only34/male'  # 深色公蛋存放目录

    # 执行函数
    getsex202412156(root_path, source_folder, destination_folder3,
                    destination_folder4)
