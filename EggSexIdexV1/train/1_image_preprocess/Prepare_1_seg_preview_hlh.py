...
# 手动标定的中心点坐标，基于左下角为 (0, 0) 的坐标系 6*7
# IMAGES_CENTER = [
...
def SegEgg(path):
    try:
        ...
        for x_core, y_core in centers:
            x1, y1 = max(0, x_core - 10), max(0, y_core - 10)
            x2, y2 = min(359, x_core + 10), min(480, y_core + 10)
        ...
        for i, (x_core, y_core) in enumerate(centers, 1):
            ...
            cv2.putText(preview_img, f"{i}", (x_core + 10, y_core - 10),
        ...
        for i, (x1, y1, x2, y2) in enumerate(sorted_area_info, 1):
            ...
            sliced = window_transposed[:, :, 100:250] # 85-235:570-870nm  100-250:600-900nm 50-100:500-700nm
            res = sliced.reshape(-1, sliced.shape[-1])

            # 检查 res 是否符合预期形状 (400, 150)
            if res.shape != (400, 150):
                print(f"警告: {data_index}-{i} 的数据形状异常: {res.shape}，预期: (400, 150)，跳过保存")
                continue
...
if __name__ == '__main__':
    preview_dir = './egg_seg_previews_hlh13d12h8000'
    data_path = r"D:\workspace\gdv-egg-model\Code_EggGenderDet\data_hlh_train_20250907_hy\raw_all_0907_13d0h_4000"
    write_path = r"D:\workspace\gdv-egg-model\Code_EggGenderDet\data_hlh_train_20250907_hy\seg_all_0907_13d0h_4000"
