...
def to_torchScript():
    ...
    root_path = r'D:\workspace\gdv-egg-model\Code_EggGenderDet\1DCNN\MQ1215_50_g_400epoch\xiaoming_hlh\0'
    try:
        model = HyperspectralCNN(num_bands=150, use_attention_pooling=True).to(device)
        model.load_state_dict(torch.load(os.path.join(root_path, '1DCNN_acc_0.937931_vloss_0.320229.pth')))
        torch.jit.script(model).save(os.path.join(root_path, 'HyLine_xm_Acc937.pt'))
    except Exception as e:
        print(f"模型转换失败: {e}")
...
if __name__ == '__main__':
    ...
    female_samples, male_samples = 3659, 3653
    ...
    fold = r'D:\workspace\gdv-egg-model\Code_EggGenderDet\1DCNN_200epoch_SNV\mixed_811_hlh_T'  # 母多公少，half，16*16
    EggNetMain()
